...
7. Body (all chapters) -- after each chapter's body paragraphs, if has_enrichments:
   ...
   - Bulleted prayer points (12pt Georgia, using LevelFormat.BULLET numbering config -- NOT Unicode bullet characters)
8. About the Author
9. Scripture Index
10. Glossary
```
...
Update the Stage 5 section to mention that the formatter now also reads enrichment files and foreword.
  ...
  <verify>
    <automated>grep -q "Stage 4.5" skills/orchestrator/SKILL.md && grep -q "book-crafter:enricher" skills/orchestrator/SKILL.md && grep -q "enrichments" skills/formatter/SKILL.md && grep -q "parseEnrichmentMarkdown" skills/formatter/SKILL.md && grep -q "foreword" skills/formatter/SKILL.md && grep -q "Stage 4.5" references/pipeline-stages.md && grep -q "Content Enrichment" references/pipeline-stages.md && grep -q "prayer-bullets" skills/formatter/SKILL.md && echo "PASS"</automated>
  </verify>
  <acceptance_criteria>
    ...
    - skills/formatter/SKILL.md contains `renderEnrichmentParagraphs` function using `numbering: { reference: "prayer-bullets", level: 0 }` -- NOT Unicode bullet characters
    - skills/formatter/SKILL.md contains `prayer-bullets` numbering config with `LevelFormat.BULLET`
    ...
    - NO Unicode bullet character (\u2022) used as TextRun text for prayer points (per CLAUDE.md)
  </acceptance_criteria>
  <done>Orchestrator has Stage 4.5 wired between editing and formatting. Formatter renders enrichments inline after each chapter and foreword in front matter. Prayer point bullets use proper LevelFormat.BULLET numbering config (not Unicode bullets). Pipeline-stages.md documents Stage 4.5 with completion detection.</done>
</task>
...
4. `skills/formatter/SKILL.md` uses `LevelFormat.BULLET` numbering config for prayer points -- no Unicode bullet characters as TextRun text
5. `references/pipeline-stages.md` documents Stage 4.5 with completion detection
6. Pipeline flow: Stage 4 (Edit) -> approve -> Stage 4.5 (Enrich) -> Stage 5 (Format) is correctly wired
7. Formatter handles both enriched projects (Phase 6+) and non-enriched projects (backward compatible)
</verification>
...
- Formatter renders discussion questions (numbered), summaries (italic), prayer points (bulleted via LevelFormat.BULLET numbering) after each chapter body
...
- No Unicode bullet characters used as TextRun text (per CLAUDE.md compliance)
</success_criteria>
