...
- [x] **PKG-07**: `scripts/release.sh` — whitelist-based zip builder (explicit allow-list, never `zip -r .`), version gate, CHANGELOG gate, size check (fail >5MB), personal-path grep gate (fail if output contains `/Users/David`)
- [x] **PKG-08**: `claude plugin validate .` passes cleanly on the packaged plugin
