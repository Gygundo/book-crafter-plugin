...
3. **Write the complete artefact** to the file. Do not write partial artefacts. **Prepend `<!-- generated-by: book-crafter v1.1.0 -->` as the first line of each `research/ch[NN]-research.md` file** (above the `# Research: Chapter [N]` heading). The version stamp is required on every generated artefact so Phase 12 regression tooling can anchor comparisons; the formatter strips all HTML comments before .docx emission.
